...
std::vector<MOTOR_PINS> motorPins =
    {
        ...
        {5, 1, 3},   // LEFT_MOTOR Pins (EnB, IN3, IN4) // Changed pinEn from 2 to 5
};
#define LIGHT_PIN 4
...
void setup()
{
    ...
    config.pin_d0 = Y2_GPIO_NUM;
    config.pin_d1 = Y3_GPIO_NUM;
    // Configure other pins based on your module
    ...
    setUpPinModes(); // Use setUpPinModes to configure pins
    setupCamera();    // Ensure camera is initialized
}

void loop()
{
    // Capture and send camera frame if client is connected
    if (cameraClientId != 0)
    {
        camera_fb_t *fb = esp_camera_fb_get();
        if (fb)
